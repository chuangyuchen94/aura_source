...
def data_pipeline_xgboost(num_cols, cat_cols):
    from sklearn.impute import SimpleImputer
    from sklearn.preprocessing import OneHotEncoder
    from sklearn.compose import ColumnTransformer
    from sklearn.pipeline import Pipeline
    ...
    model = XGBRegressor(n_estimators=1000, learning_rate=0.001, early_stopping_rounds=5, n_jobs=8)
...
if "__main__" == __name__:
    ...
    pipeline = data_pipeline_xgboost(num_cols, cat_cols)
    fit_params = {
        "model and predict__eval_set": [(X_valid, y_valid_full)],  # 验证数据集
        "model and predict__verbose": False,  # 是否打印训练过程
    }

    pipeline.fit(X_train, y_train_full, **fit_params)
